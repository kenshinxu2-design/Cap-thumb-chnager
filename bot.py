...
#  FSUB
# ══════════════════════════════════════════════════════════════════════════════
async def has_join_request(client: Client, chat_id: int, user_id: int) -> bool:
    """Check if user has a PENDING join request in channel (not yet approved)."""
    try:
        async for req in client.get_chat_join_requests(chat_id):
            if req.user.id == user_id:
                log.info(f"Found pending join request: uid={user_id} in chat={chat_id}")
                return True
    except Exception as e:
        log.warning(f"get_chat_join_requests [{chat_id}]: {e}")
    return False
    ...
    for ch in cfg.get("fsub_channels", []):
        ...
        try:
            ...
            if status in ("member", "administrator", "creator", "owner"):
                log.info(f"FSub OK: uid={uid} in {ch['name']} status={status}")
            else:
                out.append(ch)
        except UserNotParticipant:
            if ch_type == "private_request":
                # ✅ KEY LOGIC: For request-type channels,
                # if user has sent a join request (pending) → treat as PASSED
                # User ne request bheji = files milegi, approve ka wait nahi
                pending = await has_join_request(client, ch["id"], uid)
                if pending:
                    log.info(f"FSub PASS (pending request): uid={uid} in {ch['name']}")
                    # Don't add to violations — user gets files
                    continue
            # User hasn't joined AND no pending request
            out.append(ch)
            log.info(f"FSub FAIL: uid={uid} not in {ch['name']}")
        except Exception as e:
            log.warning(f"FSub check error [{ch['id']}]: {e}")
    return out
        ...
        try:
            ...
            if ch_type == "public":
                ...
                if chat.username:
                    ...
                    # Fallback: create normal invite link
                    inv = await client.create_chat_invite_link(ch["id"])
                    url = inv.invite_link
                btn_text = f"📡 Join — {name}"

            elif ch_type == "private_request":
                # Private channel — REQUEST to join (admin must approve)
                # This creates a special link where user can only REQUEST, not join directly
                try:
                    inv = await client.create_chat_invite_link(
                        ch["id"],
                        creates_join_request=True,   # Pyrofork parameter
                        name=f"FSub_{ch['id']}"      # Optional label
                    )
                    url = inv.invite_link
                except Exception as e1:
                    log.warning(f"creates_join_request failed for {ch['id']}: {e1} — trying alternate")
                    try:
                        # Some pyrofork versions use different param
                        inv = await client.create_chat_invite_link(
                            ch["id"],
                        ...
                        log.error(f"Both attempts failed {ch['id']}: {e2}")
                        inv = await client.create_chat_invite_link(ch["id"])
                        url = inv.invite_link
                btn_text = f"📨 Request to Join — {name}"

            else:
                # private — direct join via fresh invite link
                inv = await client.create_chat_invite_link(ch["id"])
                url = inv.invite_link
                btn_text = f"🔗 Join — {name}"

            rows.append([InlineKeyboardButton(btn_text, url=url)])
            log.info(f"FSub button created: {name} | type={ch_type} | url={url[:40]}...")

        except Exception as e:
            log.error(f"FSub KB FAILED [{ch.get('id')}]: {e}")
...
# ══════════════════════════════════════════════════════════════════════════════
async def forward_one(client: Client, uid: int, mid: int) -> int | None:
    """Forward single message, hide identity, return sent msg id."""
    for attempt in range(3):
        try:
            ...
            return (fw if not isinstance(fw, list) else fw[0]).id
        except FloodWait as e:
            log.warning(f"FloodWait {e.value}s on msg {mid}")
            await asyncio.sleep(e.value + 1)
        except Exception as e:
            log.warning(f"forward_one [{mid}] attempt {attempt+1}: {e}")
...
async def deliver(client: Client, user: User, encoded: str):
    ...
    if decoded.startswith("file_"):
        ...
        fid = await forward_one(client, uid, mid)
        if fid:
            sent.append(fid)
        else:
            return await send(client, uid, "<b>❌ RETRIEVAL FAILED. Try again.</b>")

    # ── New batch (MongoDB stored, exact ordered IDs) ─────────────
    elif decoded.startswith("bdoc_"):
        batch_doc_id = decoded[5:]
        doc = await _batches.find_one({"_id": batch_doc_id})
        if not doc:
            return await send(client, uid, "<b>❌ BATCH NOT FOUND — Link expired.</b>")

        msg_ids = doc.get("message_ids", [])

        # Forward STRICTLY one by one in exact order — no bulk (bulk breaks order)
        for mid in msg_ids:
            fid = await forward_one(client, uid, mid)
            if fid:
                sent.append(fid)
            # tiny delay to avoid flood but keep it fast
            await asyncio.sleep(0.05)

    # ── Legacy batch (range-based, old links) ────────────────────
...
async def cmd_add_fsub(client: Client, message: Message):
    ...
    if len(p) < 3:
        return await reply(message,
            ...
            "<b>Channel Types:</b>\n"
            "• <code>public</code> — Public channel (join via username link)\n"
            "• <code>private</code> — Private channel (fresh invite link, direct join)\n"
            "• <code>request</code> — Private channel (<b>Request to Join</b> — user needs YOUR approval)\n\n"
            "<b>Example:</b>\n"
            "<code>/add_fsub -1001234567890 request</code>\n\n"
            "<b>⚠️ IMPORTANT — For <code>request</code> type:</b>\n"
            "<blockquote>"
            "1. Make bot ADMIN in that channel\n"
            "2. Give bot <b>Invite Users via Link</b> permission\n"
            "3. Users click Request to Join button\n"
            "4. YOU approve them manually in channel\n"
            "5. Then they click Verify to get files"
            "</blockquote>")
    try:
